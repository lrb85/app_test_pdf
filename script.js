document.addEventListener('DOMContentLoaded', () => {
    ...
    let appState = {
        ...
        exams: {},
        settings: {
            timeLimit: 0, // <-- CAMBIO APLICADO: Valor por defecto es "Sin Límite"
    ...
    const handleNavClick = (viewId) => {
        ...
        const renderQuestion = () => {
            ...
            // <-- CAMBIO APLICADO: Comprobar si esta pregunta ya fue revelada
        ...
        const selectAnswer = (optionDiv) => {
            const test = appState.currentTest;
            // <-- CAMBIO APLICADO: Si la respuesta para esta pregunta ya se mostró, no hagas nada.
            if (test.answersRevealed[test.currentIndex]) {
                return;
            }
        ...
        const showAnswer = () => {
            ...
            // <-- CAMBIO APLICADO: 1. Marcar esta pregunta como revelada
            test.answersRevealed[test.currentIndex] = true;

            // <-- CAMBIO APLICADO: 2. Colorear las opciones
            optionsContainer.querySelectorAll('.option').forEach(opt => {
                ...
                // <-- CAMBIO APLICADO: 3. Desactivar la interacción
                optionsContainer.querySelectorAll('.option').forEach(opt => {
